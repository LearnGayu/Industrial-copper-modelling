...
class option():
    ...
    item_type_v=['W', 'WI', 'S', 'Others', 'PL', 'IPL', 'SLAWR']
    
    country_values=[ 25.,  26.,  27.,  28.,  30.,  32.,  38.,  39.,  40.,  77.,  78., 79.,  80.,  84.,  89., 107., 113.]

    item_type_s = {'W':5.0, 'WI':6.0, 'S':3.0, 'Others':1.0, 'PL':2.0, 'IPL':0.0, 'SLAWR':4.0}

    status_values=['Won', 'Draft', 'To be approved', 'Lost', 'Not lost for AM','Wonderful', 'Revised',
            'Offered', 'Offerable']
    
    status_encoded = {'Lost':1, 'Won':7, 'Draft':0, 'To be approved':6, 'Not lost for AM':2,'Wonderful':8, 'Revised':5,
                    'Offered':4, 'Offerable':3}
...
if selected == 'Prediction':
    title_text = '''<h1 style='font-size: 32px;text-align: center;color:white;'>Prediction of copper Selling Price and Status  </h1>'''
    ...
    if select == 'Selling Price':
        st.markdown("<h5 style=color:white>Provide the required details below:",unsafe_allow_html=True)
        st.write('')
        ...
        with st.form('prediction'):
            ...
            with col1:
                ...
                item_type=st.selectbox(label='Item Type',options=option.item_type_v,index=None)
        ...
        if button:
            ...
            if not all([item_date, delivery_date, country, item_type, application, product_ref,
                ...
                with open('C:/Users/LENOVO/4.copper modelling/Regressormodelfile.pkl','rb') as files:
                    predict_model=pickle.load(files)

                # customize the user data to fit the feature 
                status=option.status_encoded[status]
                item_type=option.item_type_s[item_type]
    ...
    if select == 'Status':
        ...
        with st.form('classifier'):
            ...
            with col1:
                ...
                item_type=st.selectbox(label='Item Type',options=option.item_type_s,index=None)
        ...
        if button:
            ...
            if not all([item_date, delivery_date, country, item_type, application, product_ref,
                ...
                with open('C:/Users/LENOVO/4.copper modelling/Classifiermodel.pkl','rb') as files:
...
if selected == "Details":
    ...
    st.markdown('''<li>Identify the types of variables (continuous, categorical) and their distributions.handling null values.''',unsafe_allow_html=True)
    ...
    st.markdown('''<li>Handle missing values with mean/median/mode.
                ...
                <li>Identify Skewness in the dataset and treat skewness with appropriate data transformations, such as log transformation, boxcox transformation, or other techniques, to handle high skewness in continuous variables.
